...
public class SML_TConnection {
	...
	/**
	 * calculates crc16 from element 0 to length-1 as specified by DIN EN
	 * 62056-46
	 * 
	 * @param bytes
	 * @param length
				...
				if ((fourBytes & 0xffffffff) == 0x1b1b1b1b) {
					...
					if ((fourBytes & 0xffffffff) != 0x1b1b1b1b) {
						...
						if (((firstByte & 0xff) != 0x1a) || (numStuffBits > 3) || firstRead) {
							// TODO
							// logger.fine("SMLTransLayerHandler: Termination sequence is wrong");
							os.close();
							bs.close();
							is.close();
							socket.close();
						...
						if (calculatedCRC != receivedCRC) {
							// TODO throw exception saying that CRC is wrong
							// logger.fine("CRC is wrong");
							// logger.finer("caluculatedCRC: " + calculatedCRC);
							// logger.finer("receivedCRC: " + receivedCRC);
	...
	/**
	 * tries to decode all SML_Messages in smlPacket sent over socket gives the
	 * message to handleSMLMessage
	 * 
	 * @param smlPacket
	 *            byteArray of SML_Messages (could be one message, one file or
	 *            several messages but no complete file)
	 * @param socket
	 *            receive socket
	 * @throws IOException
	 */
	...
	/**
	 * Interprets the type of message and handle them. Packages the publicOpen,
	 * publicClose and the messages between to one SML_File and redirects the
	 * file to the ReceiveMessageListener if publicClose was received. Sends an
	 * attentionResponse, if a file doesn't begin with a publicOpen. If a
	 * attentionResponse was received, it is redirected to the
	 * ReceiveMessageListener immediately.
	 * 
	 * @param message
	...
	private SML_File handleSMLMessage(SML_Message message, Socket socket) throws IOException {
		...
		if (tag == SML_MessageBody.OpenResponse || tag == SML_MessageBody.OpenRequest) {
			// TODO eventuell vorher abfragen ob die vorherigen Dateil schon
			// geschlossen wurde
		...
		if (smlFile == null) {
			...
			// TODO
			// logger.fine("SML_Message without PublicOpen* before received! send AttentionResponse");
